    ...
    protected override registerHooks()
    {
        this.invokeFun = (iid, f, base, _args, result) =>
        {
            ...
            if (base instanceof Dir)
            {
                ...
                {
                    FileLogStoreAdaptor.appendFileOperation(base.path, 'read', 'start', 'content', this.getSandbox(), iid);
                    assert.ok(util.types.isPromise(result));
                    result.finally(() => FileLogStoreAdaptor.appendFileOperation(base.path, 'read', 'finish', 'content', this.getSandbox(), iid));
                }
            }
